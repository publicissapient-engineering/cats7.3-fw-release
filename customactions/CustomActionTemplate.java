...
import com.sapient.qa.cats.core.framework.CATSReportGenerator;
...
public class CustomActionTemplate extends Action {
	...
	public ActionResultVO execute() throws Exception {
		...
		try {
			...
			// user can explicitly pass the file path, link to this file will appear in the
			// detail report against this action
	...
	public RemoteWebDriver getDriver(String suiteName) {
		if (null != driver && !driver.toString().contains("null")) {
			int implicitWaitTime = CATSReportGenerator.getInstance(suiteName).getImplicitWaitTime();
